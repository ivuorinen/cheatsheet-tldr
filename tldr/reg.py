...
`reg.py {{domain}}/{{username}}:{{password}}@{{target}} delete -keyName '{{HKLM\SOFTWARExample}}' -v {{value_name}}`

- Save a registry key (and subkeys) to a file on the target via UNC path:

`reg.py {{domain}}/{{username}}:{{password}}@{{target}} save -keyName '{{HKLM\SOFTWARExample}}' -o '\{{target}}\{{share}}\{{output_file.reg}}'`
